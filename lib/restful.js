...
var make_new_object = function(reqBody, schema){
    var newObj = {};
    for(var key in schema){
        var type = schema[key];
        if(typeof(type) === 'object'){
            //newObj[key] = make_new_object(reqBody, type);
        }
        else if(typeof(type) == 'function'){
            var funcRetType = type.toString().substring(type.toString().indexOf(' ') + 1, type.toString().indexOf('('));


            if(funcRetType == 'Date') {
                if (reqBody[key] && reqBody[key].year && reqBody[key].month && reqBody[key].day) {
                    newObj[key].year = reqBody[key].year;
                    newObj[key].month = reqBody[key].month;
                    newObj[key].day = reqBody[key].day;
                }
            }
